package Koha::Plugin::Se::Gu::Ub::EdifactUb::Order;
...
sub order_msg_header {
    ...
    my $branch_delivery_address = ''; # Empty string as default 
    if ($self->{plugin}->retrieve_data($self->{basket}->deliveryplace)) {
      if ($self->{basket}->deliveryplace eq $self->{basket}->billingplace) {
        # this is not a c/o address
        $branch_delivery_address .= get_branch_name($self->{basket}->billingplace) . '+';
      }
      else {
        #this is a c/o address
        $branch_delivery_address .= get_branch_name($self->{basket}->billingplace) . ' c/o ' . get_branch_name($self->{basket}->deliveryplace) . '+';
      }
      $branch_delivery_address .= $self->{plugin}->retrieve_data($self->{basket}->deliveryplace);

      $branch_delivery_address =~ s/\r\n/+/g;
    ...
    push @header,
      name_and_address(
        'DELIVERY',
        $self->{basket}->billingplace . '-' . $self->{basket}->deliveryplace . '::92',
        $branch_delivery_address
      );
